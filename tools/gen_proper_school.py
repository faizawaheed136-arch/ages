#!/usr/bin/env python3
"""Bakes the NEW school's exterior shell into assets/ProperSchool.rbxmx.

A near-copy of gen_school.py -- same bootcheck harness, same recording Instance.new, same
rbxmx emitter -- pointed at ProperSchool/Exterior.luau instead of world/School.luau. Kept as a
separate file rather than a flag on the old one because the two schools are meant to coexist
while the new one is built, and a single tool that could bake either is a tool that will one
day bake the wrong one over the other.
...
OUT = ROOT / "assets" / "ProperSchool.rbxmx"

# Centre of the new school, in world coordinates. See the note in main().
SITE = (-790.0, 0.0, -630.0)
...
-- Required directly rather than looked up in `order`.
--
-- `moduleNamed` searches the modules bootcheck actually loaded, and bootcheck loads lazily --
-- a module only appears there once something in the game requires it. Nothing requires the new
...
-- A plain table stands in for the folder the shell is parented into. Exterior.Build takes an
...
-- No room anchors: this is the exterior shell, it has no rooms yet, and the map's place points
-- all belong to school v1 and must stay with it (relocate_place_points is disabled in main).
local plan = nil
local anchors = {}
if plan ~= nil then
...
def record() -> tuple[list[dict], list[dict]]:
    ...
    # The new school's site, surveyed clear on a 20-stud occupancy grid across every asset:
    # 260x180 free, 247 studs from school v1 so both can be walked in one trip. Written here
    # rather than read from a place point because there is no marker for it yet -- the place
    # points in Street.rbxmx all belong to v1 and must not be moved onto this building.
    px, py, pz = SITE
...
def emit(rows: list[dict]) -> None:
    ...
    print(rbxmx.write(OUT, "ProperSchool"))
...
if __name__ == "__main__":
    ...
    # relocate_place_points is v1's -- its markers stay with v1.
